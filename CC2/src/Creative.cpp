...
#include <cstdlib>
#include <time.h> // Used for the purpose of psuedo-random number generation
...
int main() {
    ...
    std::srand((unsigned int)time(NULL)); // Initializes rand() based on current time
    for (int i = 0; i < numStudents; ++i) {
        int temp =  rand() % 30;
        COUT << temp << ENDL;
        students.push_back(NAMES.at(temp));
    }
